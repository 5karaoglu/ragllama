...
logger = setup_logging("json_rag")

class JsonRagModule:
    """JSON verileri ile çalışan RAG modülü."""
    
    def __init__(self, json_file: str = "Book1.json", persist_dir: str = "./storage/json"):
        """
        JSON RAG modülünü başlatır.
        
        Args:
            json_file (str): JSON veri dosyasının yolu
    ...
    def initialize(self, llm: Optional[HuggingFaceLLM] = None):
        ...
        """
        # JSON verisini yükle
        self.json_data = self.load_json_data(self.json_file)
        
        # JSON indeksini oluştur veya yükle
        self.json_rows = self.create_or_load_json_index(self.json_data, self.persist_dir)
        
        # Query engine oluştur
        self.query_engine = JSONalyzeQueryEngine(
            list_of_dict=self.json_rows,
            llm=llm or Settings.llm,
            verbose=True
        )
        
        logger.info("JSON RAG modülü başarıyla başlatıldı.")
        return self
    ...
    def query(self, query_text: str) -> str:
        ...
        logger.info(f"JSON RAG sorgusu işleniyor: {query_text}")
    ...
    def load_json_data(self, file_path: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Yüklenen JSON verisi
        """
    ...
    def create_or_load_json_index(self, json_data: Dict[str, Any], persist_dir: str) -> List[Dict[str, Any]]:
        """
        JSON indeksini oluşturur veya yükler.
        
        Args:
            json_data (Dict[str, Any]): JSON verisi
            persist_dir (str): İndeksin kaydedileceği dizin
            
        Returns:
            List[Dict[str, Any]]: Düzleştirilmiş JSON satırları
        """
        # Dizin varsa yükle, yoksa oluştur
        if os.path.exists(persist_dir) and os.path.exists(os.path.join(persist_dir, "json_rows.json")):
            ...
            try:
                ...
                logger.info("JSON satırları başarıyla yüklendi.")
                return json_rows
            except Exception as e:
                ...
                return self.create_new_json_index(json_data, persist_dir)
        else:
            logger.info("Yeni JSON indeksi oluşturuluyor...")
            return self.create_new_json_index(json_data, persist_dir)
    
    def create_new_json_index(self, json_data: Dict[str, Any], persist_dir: str) -> List[Dict[str, Any]]:
        """
